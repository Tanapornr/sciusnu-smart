...
// In production, replace with Redis or a DB-backed set.
const revokedTokens = new Set();
app.revokedTokens = revokedTokens; // expose for middleware in lib/auth.js if needed

// Logout — adds the token's JTI/exp to blocklist so it can't be reused
app.post("/api/logout", (req, res) => {
  ...
  if (token) {
    ...
    if (decoded) {
      // Key by token itself (or decoded.jti if you add jti to signToken)
      revokedTokens.add(token);
      // Auto-clean after token's natural expiry to bound memory growth
